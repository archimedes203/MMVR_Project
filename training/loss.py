...
class PoseLoss(nn.Module):
    ...
    - L_heatmap  : MSE between predicted and GT Gaussian heatmaps.
                   This is weighted higher because final coordinates are
                   inferred from the model's heatmap shape.
    - L_coord    : SmoothL1 between predicted and GT (x,y) coordinates,
                   weighted by keypoint visibility.

    # ── Gaussian NLL variant (DISABLED) ────────────────────────────────────
    ...
    def __init__(self, lambda_hm=5.0, lambda_coord=1.0):
        super().__init__()
        self.lambda_hm    = lambda_hm
        self.lambda_coord = lambda_coord
        self.smooth_l1    = nn.SmoothL1Loss(reduction='none')
        # self.gaussian_nll = nn.GaussianNLLLoss(full=True, reduction='none')   # Gauss NLL variant
    ...
    def forward(self, pred_hm, pred_coords, gt_hm, gt_coords, visibility):
        ...
        # Gaussian NLL variant signature:
        ...
        """
        # ── Heatmap loss: MSE against explicit Gaussian GT heatmaps ──────────
        if pred_hm.shape[2:] != gt_hm.shape[2:]:
            gt_hm = F.interpolate(gt_hm, size=pred_hm.shape[2:],
                                  mode='bilinear', align_corners=False)
        loss_hm = F.mse_loss(pred_hm.sigmoid(), gt_hm)
        ...
        # ── Coordinate loss: Gaussian NLL with learned variance — DISABLED ───
